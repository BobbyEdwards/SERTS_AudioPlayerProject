      ...
      break;
		case Pause:
			if (event == PlayFile){
				...
				// Play State Entry Actions
				LED_On(LED_Green);
				osMessagePut(mid_PlayPause_Queue, ResumeFile, 0);
			}
			break;
		case Play:
			if (event == PauseFile){
			...
			if (event == SongDone){
				...
				// Idle State Entry Actions
				//LED_On(LED_Red);
				//osMessagePut(mid_PlayPause_Queue, PauseFile, 0);	// No action
			}
			break;
    default:
      break;
			...
			switch(event.value.v) {
				...
				case ResumeFile:
					...
					while(sizeRead == 1){
						...
						event = osMessageGet(mid_PlayPause_Queue, 0); // receive command
						if (event.status == osEventMessage) { // check for valid message
